...
public class FeePaymentView extends JFrame {
	...
	public FeePaymentView() {
		...
		frame.setBounds(100, 100, 577, 654);
		 ...
		 * 
		 * @Override public void keyTyped(KeyEvent arg0) { int
		 * payable=Integer.parseInt(textPaidAmount.getText()); int
		 * paid=Integer.parseInt(textPaidAmount.getText()); if(paid>payable){
		 * JOptionPane.showMessageDialog(
		 * null,"You Cannot Pay More Than Payable Amount"); } } });
		 */
		...
		btnPayableAmount.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!textDiscount.getText().equals("")) {
					discount = Integer.parseInt(textDiscount.getText());
				...
				if (res < 3000) {
					JOptionPane.showMessageDialog(null,
							"Payable Amount Cannot Be Less Than " + "3000.Check the Discount Offered to this Student ");
		 ...
		 * contentPane.add(lblChequeNum);
		 * 
		 * textChequeNum = new JTextField(); textChequeNum.setFont(new
		 * Font("Times New Roman", Font.PLAIN, 12));
		 * textChequeNum.setBounds(176, 449, 154, 20);
		 * contentPane.add(textChequeNum); textChequeNum.setColumns(10);
		 */
		clock();
		fillComboBox();
	...
	private void search() {
		...
		try {
			if (rs.next()) {
				textStudentName.setText(rs.getString("Name"));
				textCourseName.setText(rs.getString("Course"));
				textTotalFee.setText(rs.getString("TotalFees"));
				textDiscount.setText(rs.getString("Discount"));
			} else {
				JOptionPane.showMessageDialog(null, "Wrong enrollment number");
	...
	private void feePayment() {
		...
		FeePayamentDAO feePaymentDAO = new FeePayamentDAO();
		FeePaymentDTO feePaymentDTO = new FeePaymentDTO();
		feePaymentDTO.setAdmissionNum(textAdmissionnum.getText());
	...
	private void generate() {
		...
		try {
			PdfWriter writer = PdfWriter.getInstance(document,
					new FileOutputStream(textStudentName.getText() + ".pdf"));
			document.open();
			...
			Chunk chunk0 = new Chunk("INSTITUTE MANAGEMENT SYSTEM");
			...
			Chunk chunk1 = new Chunk("STUDENT ID:" + "    " + s1);
			Chunk chunk2 = new Chunk("STUDENT NAME: " + "    " + s2 + "                                        ");
			Chunk chunk3 = new Chunk("COURSE NAME:" + "    " + s3 + " ");
			Chunk chunk4 = new Chunk("TOTAL FEES:" + "    " + s4);
			Chunk chunk5 = new Chunk(" DISCOUNT:" + "    " + s5);
			Chunk chunk6 = new Chunk("PAYABLE AMOUNT :" + "    " + s6);
			Chunk chunk7 = new Chunk(" PAID AMOUNT :" + "    " + s7);
			Chunk chunk8 = new Chunk(" DUE AMOUNT :" + "    " + s8);
			Chunk chunk9 = new Chunk(" DATE :" + "    " + s9);
